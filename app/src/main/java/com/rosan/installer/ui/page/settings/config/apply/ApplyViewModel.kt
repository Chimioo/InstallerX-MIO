...
class ApplyViewModel(
    ...
    fun dispatch(action: ApplyViewAction) {
        when (action) {
            ...
            is ApplyViewAction.ApplyPackageName ->
                    applyPackageName(action.packageName, action.applied)
    ...
    private fun loadApps() {
        ...
        loadAppsJob =
                viewModelScope.launch(Dispatchers.IO) {
                    state =
                            state.copy(
                                    apps = state.apps.copy(progress = ViewContent.Progress.Loading)
                            )
                    ...
                    val list =
                            packageManager.getCompatInstalledPackages(0).map {
                                ApplyViewApp(
                                        ...
                                        isSystemApp =
                                                it.applicationInfo!!.flags.hasFlag(
    ...
    private fun collectAppEntities() {
        ...
        collectAppEntitiesJob =
                viewModelScope.launch(Dispatchers.IO) {
                    state =
                            state.copy(
                                    appEntities =
                                            state.appEntities.copy(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (applied) {
                if (entity != null) {
                    ...
                    appRepo.insert(AppEntity(packageName = packageName, configId = id))
                }
            } else {
